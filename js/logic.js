...
let changeState = function(event) {
	...
	if (event.target.className === 'place') {
		...
		if (!currentState.clickedPlaces.includes(event.target)) {
			...
			currentState.clickedPlaces.push(event.target);
		}
...
let doAction = function(event) {
	...
	if (victoryDetails[0] === 'draw') {
		victory.finish(doAction, changeState, finishButton);
		document.querySelector('.nextRound').insertAdjacentHTML('beforeend', '<p>НИЧЬЯ</p>');
		return;
	}
...
let reloadRound = function(event) {
	...
	currentState.clickedPlaces = [];

	if (document.querySelector('.nextRound p')) {
		document.querySelector('.nextRound p').remove();
	}
